...
# Create network
G = NetworkGlow(1, n_hidden, L, K; split_scales=true, p2=0, k2=1, activation=SigmoidLayer(low=low,high=1.0f0)) |> gpu
...
# Dummy call to to initialize actnorm
t = G.forward(X_train_latent); 
...
for e=1:nepochs
    ...
    subplot(4,5,5); PyPlot.scatter(quantiles⁰, obs, s=0.5)
